...
function calcularJacobi() {
    ...
    heads += '<th>Tolerancia</th>'; // Adaptado dinámicamente
    ...
    while (error > tol && iter < maxIter) {
        ...
        if (iter > 0) {
            ...
            for (let i = 0; i < n; i++) {
                ...
                let currVis = parseFloat(x_curr[i].toFixed(4));
                let prevVis = parseFloat(x_prev[i].toFixed(4));
                sumDiffSq += Math.pow(currVis - prevVis, 2);
            }
